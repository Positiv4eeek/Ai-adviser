<template>
  <div class="p-6 space-y-8">

    <div class="bg-zinc-800 p-6 rounded-xl text-white space-y-4">
      <h2 class="text-2xl font-bold text-center">
        {{ $t('curriculum.curriculum_metadata') }}
      </h2>
      <div class="bg-zinc-700 p-4 rounded-lg max-w-md mx-auto">
        <dl class="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
          <dt class="font-medium">{{ $t('curriculum.program_code') }}</dt>
          <dd>{{ data.program.program_code }}</dd>
          <dt class="font-medium">{{ $t('curriculum.program_name') }}</dt>
          <dd>{{ data.program.program_name }}</dd>
          <dt class="font-medium">{{ $t('curriculum.intake_year') }}</dt>
          <dd>{{ data.program.intake_year }}</dd>
          <dt class="font-medium">{{ $t('curriculum.total_credits') }}</dt>
          <dd>{{ data.program.total_credits }}</dd>
        </dl>
      </div>
    </div>


    <div class="bg-zinc-800 p-6 rounded-xl text-white space-y-4">
      <h2 class="text-2xl font-bold text-center">{{ $t('courses.courses') }}</h2>
      <div class="flex flex-wrap justify-center gap-2 mb-4">
        <button
          @click="activeSem = 'fall'"
          :class="tabClass(activeSem === 'fall')"
        >{{ $t('tabs.fall') }}</button>
        <button
          @click="activeSem = 'spring'"
          :class="tabClass(activeSem === 'spring')"
        >{{ $t('tabs.spring') }}</button>
      </div>
      <div class="flex flex-wrap justify-center gap-2 mb-4">
        <button
          v-for="y in years" :key="y"
          @click="activeYear = y"
          :class="tabClass(activeYear === y)"
        >{{ y }} {{ $t('tabs.course_short') }}</button>
      </div>

      <div class="bg-zinc-700 p-4 rounded-lg overflow-auto max-h-96">
        <table class="w-full text-sm border-collapse">
          <thead class="sticky top-0 bg-zinc-800 bg-opacity-90">
            <tr>
              <th class="px-3 py-2 font-medium text-white">{{ $t('courses.headers.index') }}</th>
              <th class="px-3 py-2 font-medium text-white">{{ $t('courses.headers.blockCode') }}</th>
              <th class="px-3 py-2 font-medium text-white">{{ $t('courses.headers.disciplineCode') }}</th>
              <th class="px-3 py-2 font-medium text-white">{{ $t('courses.headers.disciplineName') }}</th>
              <th class="px-3 py-2 font-medium text-white">{{ $t('courses.headers.disciplineType') }}</th>
              <th class="px-3 py-2 text-center font-medium text-white">{{ $t('courses.headers.prerequisite') }}</th>
              <th class="px-3 py-2 text-center font-medium text-white">{{ $t('courses.headers.credits') }}</th>
              <th class="px-3 py-2 text-center font-medium text-white">{{ $t('courses.headers.contactHours') }}</th>
              <th class="px-3 py-2 text-center font-medium text-white">{{ $t('courses.headers.examType') }}</th>
              <th class="px-3 py-2 text-center font-medium text-white">{{ $t('courses.headers.module') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="(c, idx) in data.courses[activeYear][activeSem]"
              :key="idx"
              class="odd:bg-zinc-700 even:bg-zinc-600 hover:bg-zinc-500 transition-colors"
            >
              <td class="px-3 py-2 text-center">{{ idx + 1 }}</td>
              <td class="px-3 py-2">{{ c.block }}</td>
              <td class="px-3 py-2">{{ c.discipline_code }}</td>
              <td class="px-3 py-2">{{ c.discipline_name }}</td>
              <td class="px-3 py-2">{{ c.discipline_type }}</td>
              <td class="px-3 py-2">{{ c.prerequisite }}</td>
              <td class="px-3 py-2 text-center">{{ c.credits }}</td>
              <td class="px-3 py-2 text-center">
                <span v-if="c.contact_hours !== null">{{ c.contact_hours }}</span>
                <span v-else><FontAwesomeIcon icon="fa-minus" /></span>
              </td>
              <td class="px-3 py-2 text-center">{{ c.exam_type }}</td>
              <td class="px-3 py-2 text-center">{{ c.module }}</td>
            </tr>
            <tr v-if="!data.courses[activeYear][activeSem].length">
              <td colspan="5" class="px-3 py-2 text-center italic">
                {{ $t('general.no_courses') }}
              </td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>


    <div v-if="groups.length" class="bg-zinc-800 p-6 rounded-xl text-white space-y-4">
      <h2 class="text-2xl font-bold text-center">{{ $t('electives.electives') }}</h2>
      <div class="flex flex-wrap justify-center gap-2 mb-4">
        <button
          v-for="grp in groups" :key="grp"
          @click="activeGroup = grp"
          :class="tabClass(activeGroup === grp)"
        >{{ $t('electives.group') }} {{ grp }}</button>
      </div>
      <div class="bg-zinc-700 p-4 rounded-lg overflow-auto max-h-80">
        <table class="w-full text-sm border-collapse">
          <thead class="sticky top-0 bg-zinc-800 bg-opacity-90">
            <tr>
              <th class="px-3 py-2 font-medium text-white">{{ $t('courses.headers.index') }}</th>
              <th class="px-3 py-2 font-medium text-white">{{ $t('courses.headers.blockCode') }}</th>
              <th class="px-3 py-2 font-medium text-white">{{ $t('courses.headers.disciplineCode') }}</th>
              <th class="px-3 py-2 font-medium text-white">{{ $t('courses.headers.disciplineName') }}</th>
              <th class="px-3 py-2 font-medium text-white">{{ $t('courses.headers.disciplineType') }}</th>
              <th class="px-3 py-2 text-center font-medium text-white">{{ $t('courses.headers.prerequisite') }}</th>
              <th class="px-3 py-2 text-center font-medium text-white">{{ $t('courses.headers.credits') }}</th>
              <th class="px-3 py-2 text-center font-medium text-white">{{ $t('courses.headers.contactHours') }}</th>
              <th class="px-3 py-2 text-center font-medium text-white">{{ $t('courses.headers.examType') }}</th>
              <th class="px-3 py-2 text-center font-medium text-white">{{ $t('courses.headers.module') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="(e, idx) in data.electives[activeGroup]"
              :key="idx"
              class="odd:bg-zinc-700 even:bg-zinc-600 hover:bg-zinc-500 transition-colors"
            >
              <td class="px-3 py-2 text-center">{{ idx + 1 }}</td>
              <td class="px-3 py-2">{{ e.block }}</td>
              <td class="px-3 py-2">{{ e.discipline_code }}</td>
              <td class="px-3 py-2">{{ e.discipline_name }}</td>
              <td class="px-3 py-2">{{ e.discipline_type }}</td>
              <td class="px-3 py-2">{{ e.prerequisite }}</td>
              <td class="px-3 py-2 text-center">{{ e.credits }}</td>
              <td class="px-3 py-2 text-center">
                <span v-if="e.contact_hours !== null">{{ e.contact_hours }}</span>
                <span v-else>—</span>
              </td>
              <td class="px-3 py-2 text-center">{{ e.exam_type }}</td>
              <td class="px-3 py-2 text-center">{{ e.module }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
  </div>
</template>

<script setup>
import { computed, ref, watch } from 'vue'
import { useI18n } from 'vue-i18n'

const props = defineProps({
  data: {
    type: Object,
    required: true
  }
})

const { t } = useI18n()


const years      = computed(() => Object.keys(props.data.courses))
const activeYear = ref(years.value[0] || '')
watch(years, v => { if (v.length) activeYear.value = v[0] })

const activeSem = ref('fall')


const groups      = computed(() => Object.keys(props.data.electives))
const activeGroup = ref(groups.value[0] || '')
watch(groups, v => { if (v.length) activeGroup.value = v[0] })

const tabClass = isActive => [
  'px-4 py-2 rounded text-white font-semibold transition',
  isActive
    ? 'bg-blue-600 ring-2 ring-white'
    : 'bg-black hover:bg-opacity-80'
]
</script>

<style scoped>
.overflow-auto::-webkit-scrollbar {
  height: 10px;
}
.overflow-auto::-webkit-scrollbar-thumb {
  background-color: rgba(255,255,255,0.2);
  border-radius: 3px;
}
</style>
